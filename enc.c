...
void sparse_encode(char *sblk, char *cblk) {
    int i, j;
    ...
    for (j = M; j < N; j++) {
        ...
        if (sblk[j - M] == 1) {
            for (e = mod2sparse_first_in_col(H, cols[j]); !mod2sparse_at_end(e);
...
void dense_encode(char *sblk, char *cblk, mod2dense *u, mod2dense *v) {
    int j;
...
void mixed_encode(char *sblk, char *cblk, mod2dense *u, mod2dense *v) {
    mod2entry *e;
    int j;
