...
int main(int argc, char **argv) {
    ...
    if (!(pchk_file = argv[1]) || !(gen_file = argv[2]) || !(meth = argv[3])) {
        usage();
    }
    ...
    if (strcmp(meth, "sparse") == 0) {
        ...
        if (argv[4]) {
            ...
            else if (strcmp(argv[4], "mincol") == 0)
                strategy = Mod2sparse_mincol;
            else if (strcmp(argv[4], "minprod") == 0)
                strategy = Mod2sparse_minprod;
            else {
                usage();
            }
            if (argv[5]) {
                if (sscanf(argv[5], "%d%c", &abandon_number, &junk) != 1 ||
                    abandon_number <= 0 || !argv[6] ||
                    sscanf(argv[6], "%d%c", &abandon_when, &junk) != 1 ||
                    abandon_when <= 0 || argv[7]) {
                    usage();
                }
            }
        }
    ...
    if (N <= M) {
        fprintf(stderr,
                "Can't encode if number of bits (%d) isn't greater than number "
                "of checks (%d)\n",
                N, M);
        exit(1);
    }
...
void make_dense_mixed(FILE *f, make_method method, char *other_gen_file) {
    ...
    if (other_gen_file) {
        ...
        if (!mod2dense_invert(A, AI)) {
            fprintf(stderr, "Couldn't invert sub-matrix with column order "
                            "given in other file\n");
            exit(1);
        }
    ...
    if (method == Mixed) {
        ...
        fprintf(stderr,
                "Number of 1s per check in Inv(A) is %.1f, in B is %.1f, total "
                "is %.1f\n",
...
void make_sparse(FILE *f, mod2sparse_strategy strategy, int abandon_number,
                 int abandon_when) {
    int n, cL, cU, cB;
    int i;
    ...
    n = mod2sparse_decomp(H, M, L, U, rows, cols, strategy, abandon_number,
                          abandon_when);
    ...
    if (n != 0 && abandon_number > 0) {
        fprintf(stderr,
                "Note: Have %d dependent columns, but this could be due to "
                "abandonment.\n",
                n);
    ...
    for (i = 0; i < M; i++)
        cL += mod2sparse_count_row(L, i);
    for (i = 0; i < M; i++)
        cU += mod2sparse_count_row(U, i);
    for (i = M; i < N; i++)
        cB += mod2sparse_count_col(H, cols[i]);

    fprintf(stderr,
            "Number of 1s per check in L is %.1f, U is %.1f, B is %.1f, total "
            "is %.1f\n",
            (double)cU / M, (double)cL / M, (double)cB / M,
            (double)(cL + cU + cB) / M);
...
void usage(void) {
    fprintf(stderr, "Usage:  make-gen pchk-file gen-file method\n");
    fprintf(stderr, "Method: sparse [ \"first\" | \"mincol\" | \"minprod\" ] [ "
                    "abandon_num abandon_when ]\n");
