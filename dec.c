...
void enum_decode_setup(void) {
    ...
    if (N - M > 31) {
        fprintf(stderr,
                "Trying to decode messages with %d bits by exhaustive "
                "enumeration is absurd!\n",
                N - M);
        exit(1);
    }
...
void prprp_decode_setup(void) {
    if (table == 2) {
        printf("  block  iter  changed  perrs    loglik   Eperrs   Eloglik  "
               "entropy\n");
    }
}
    ...
    for (n = 0;; n++) {
        ...
        if (table == 2) {
            printf("%7d %5d %8.1f %6d %+9.2f %8.1f %+9.2f  %7.1f\n", block_no,
                   n, changed(lratio, dblk, N), c,
                   loglikelihood(lratio, dblk, N),
                   expected_parity_errors(H, bprb),
                   expected_loglikelihood(lratio, bprb, N), entropy(bprb, N));
        }
...
void iterprp(
    ...
    for (i = 0; i < M; i++) {
        dl = 1;
        for (e = mod2sparse_first_in_row(H, i); !mod2sparse_at_end(e);
             e = mod2sparse_next_in_row(e)) {
            e->lr = dl;
            dl *= 2 / (1 + e->pr) - 1;
        }
        dl = 1;
        for (e = mod2sparse_last_in_row(H, i); !mod2sparse_at_end(e);
    ...
    for (j = 0; j < N; j++) {
        ...
        dblk[j] = pr >= 1;
        pr = 1;
        for (e = mod2sparse_last_in_col(H, j); !mod2sparse_at_end(e);
