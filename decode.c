...
int main(int argc, char **argv) {
    ...
    if (!(pchk_file = argv[1]) || !(rfile = argv[2]) || !(dfile = argv[3])) {
        usage();
    }
    ...
    /* Look at the specification of the decoding method, which starts at meth
       and continues to the end of the command line (marked by a zero pointer).
     */
    ...
    if ((strcmp(pchk_file, "-") == 0) + (strcmp(rfile, "-") == 0) > 1) {
        fprintf(stderr,
                "Can't read more than one stream from standard input\n");
        exit(1);
    }
    if ((table > 0) + (strcmp(dfile, "-") == 0) +
            (pfile != 0 && strcmp(pfile, "-") == 0) >
        1) {
    ...
    for (block_no = 0;; block_no++) {
        ...
        for (i = 0; i < N; i++) {
            ...
            if (c == EOF) {
                if (i > 0) {
                    fprintf(stderr,
                            "Warning: Short block (%d long) at end of received "
                            "file ignored\n",
                            i);
                }
                goto done;
            ...
            if (c < 1 ||
                channel == BSC && bsc_data[i] != 0 && bsc_data[i] != 1) {
...
void usage(void) {
    ...
    fprintf(stderr, "  decode [ -f ] [ -t | -T ] pchk-file received-file "
                    "decoded-file [ bp-file ] channel method\n");
    channel_usage();
    fprintf(stderr, "Method:  enum-block gen-file | enum-bit gen-file | prprp "
                    "[-]max-iterations\n");
    exit(1);
}
