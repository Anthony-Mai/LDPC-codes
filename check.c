...
double changed(double *lratio, /* Likelihood ratios for bits */
...
double expected_parity_errors(mod2sparse *H, /* Parity check matrix */
    ...
    for (i = 0; i < M; i++) {
        p = 0;
        for (f = mod2sparse_first_in_row(H, i); !mod2sparse_at_end(f);
...
double loglikelihood(double *lratio, /* Likelihood ratios for bits */
...
double expected_loglikelihood(double *lratio, /* Likelihood ratios for bits */
                              double *bpr,    /* Bit probabilities */
                              int N           /* Length of codeword */
) {
    double ll;
...
double entropy(double *bpr, /* Bit probabilities */
               int N        /* Length of codeword */
) {
    double e;
    int j;
