...
int main(int argc, char **argv) {
    ...
    if (!(pchk_file = argv[1]) || !(coded_file = argv[2])) {
        usage();
    }
    ...
    if ((strcmp(pchk_file, "-") == 0) + (strcmp(coded_file, "-") == 0) +
            (source_file != 0 && strcmp(source_file, "-") == 0) +
            (gen_file != 0 && strcmp(gen_file, "-") == 0) >
        1) {
        fprintf(stderr,
    ...
    for (n = 0;; n++) {
        ...
        /* Check that received block is a code word, and if not find the number
           of parity check errors. */
    ...
    if (gen_file != 0) {
        fprintf(stderr,
                "Block counts: tot %d, with chk errs %d, with src errs %d, "
                "both %d\n",
                n, tot_chkerrs, tot_srcerrs, tot_botherrs);
        fprintf(stderr, "Bit error rate (on message bits only): %.3e\n",
                (double)bit_errs / (n * (N - M)));
    } else {
        fprintf(stderr, "Block counts: tot %d, with chk errs %d\n", n,
                tot_chkerrs);
    }
...
void usage(void) {
    fprintf(stderr, "Usage: verify [ -t ] pchk-file decoded-file [ gen-file [ "
                    "source-file ] ]\n");
    exit(1);
}
