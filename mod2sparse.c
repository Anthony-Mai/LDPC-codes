...
mod2sparse *mod2sparse_allocate(int n_rows, /* Number of rows in matrix */
    ...
    if (n_rows <= 0 || n_cols <= 0) {
        fprintf(stderr,
                "mod2sparse_allocate: Invalid number of rows or columns\n");
        exit(1);
    }
...
void mod2sparse_copy(mod2sparse *m, /* Matrix to copy */
...
void mod2sparse_copyrows(mod2sparse *m, /* Matrix to copy */
                         mod2sparse *r, /* Place to store copy of matrix */
    ...
    if (mod2sparse_cols(m) > mod2sparse_cols(r)) {
        fprintf(stderr, "mod2sparse_copyrows: Destination matrix has fewer "
                        "columns than source\n");
        exit(1);
    }
...
void mod2sparse_copycols(mod2sparse *m, /* Matrix to copy */
                         mod2sparse *r, /* Place to store copy of matrix */
    ...
    if (mod2sparse_rows(m) > mod2sparse_rows(r)) {
        fprintf(stderr, "mod2sparse_copycols: Destination matrix has fewer "
                        "rows than source\n");
        exit(1);
    }
...
mod2entry *mod2sparse_insert(mod2sparse *m, int row, int col) {
    ...
    if (row < 0 || row >= mod2sparse_rows(m) || col < 0 ||
        col >= mod2sparse_cols(m)) {
        fprintf(stderr,
                "mod2sparse_insert: row or column index out of bounds\n");
        exit(1);
    }
...
void mod2sparse_add(mod2sparse *m1, /* Left operand of add */
    ...
    if (r == m1 || r == m2) {
        fprintf(stderr, "mod2sparse_add: Result matrix is the same as one of "
                        "the operands\n");
        exit(1);
    }
...
void mod2sparse_multiply(mod2sparse *m1, /* Left operand of multiply */
    ...
    if (r == m1 || r == m2) {
        fprintf(stderr, "mod2sparse_multiply: Result matrix is the same as one "
                        "of the operands\n");
        exit(1);
    }
...
void mod2sparse_mulvec(
    ...
    for (j = 0; j < N; j++) {
        if (u[j]) {
            for (e = mod2sparse_first_in_col(m, j); !mod2sparse_at_end(e);
                 e = mod2sparse_next_in_col(e)) {
...
int mod2sparse_count_row(mod2sparse *m, int row) {
    ...
    for (e = mod2sparse_first_in_row(m, row); !mod2sparse_at_end(e);
         e = mod2sparse_next_in_row(e)) {
        count += 1;
    }
...
int mod2sparse_count_col(mod2sparse *m, int col) {
    ...
    for (e = mod2sparse_first_in_col(m, col); !mod2sparse_at_end(e);
         e = mod2sparse_next_in_col(e)) {
        count += 1;
    }
...
void mod2sparse_add_row(mod2sparse *m1, /* Matrix containing row to add to */
    ...
    if (mod2sparse_cols(m1) < mod2sparse_cols(m2)) {
        fprintf(stderr, "mod2sparse_add_row: row added to is shorter than row "
                        "added from\n");
        exit(1);
    }

    if (row1 < 0 || row1 >= mod2sparse_rows(m1) || row2 < 0 ||
        row2 >= mod2sparse_rows(m2)) {
...
void mod2sparse_add_col(
    ...
    if (mod2sparse_rows(m1) < mod2sparse_rows(m2)) {
        fprintf(stderr, "mod2sparse_add_col: Column added to is shorter than "
                        "column added from\n");
        exit(1);
    }

    if (col1 < 0 || col1 >= mod2sparse_cols(m1) || col2 < 0 ||
        col2 >= mod2sparse_cols(m2)) {
...
int mod2sparse_decomp(
    ...
    for (i = 0; i < K; i++) {
        ...
        /* Get rid of all entries in the current column of B, just to save
         * space. */

        for (;;) {
            f = mod2sparse_first_in_col(B, cols[i]);
            if (mod2sparse_at_end(f))
                break;
...
int mod2sparse_forward_sub(
    ...
    for (i = 0; i < K; i++) {
        ...
        for (e = mod2sparse_first_in_row(L, ii); !mod2sparse_at_end(e);
             e = mod2sparse_next_in_row(e)) {
            j = mod2sparse_col(e);
...
int mod2sparse_backward_sub(
    ...
    for (i = K - 1; i >= 0; i--) {
        ...
        for (e = mod2sparse_first_in_row(U, i); !mod2sparse_at_end(e);
             e = mod2sparse_next_in_row(e)) {
            j = mod2sparse_col(e);
