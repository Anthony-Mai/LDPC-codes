...
int main(int argc, char **argv) {
    ...
    if (!(file = argv[1]) || !argv[2] ||
        sscanf(argv[2], "%d%c", &M, &junk) != 1 || M <= 0 || !argv[3] ||
        sscanf(argv[3], "%d%c", &N, &junk) != 1 || N <= 0 || !argv[4] ||
        sscanf(argv[4], "%d%c", &seed, &junk) != 1) {
        usage();
    }
    ...
    if (distrib_max(d) > M) {
        fprintf(stderr,
                "At least one checks per bit (%d) is greater than total checks "
                "(%d)\n",
                distrib_max(d), M);
        exit(1);
    }
    ...
    if (distrib_max(d) == M && N > 1 && no4cycle) {
        fprintf(stderr, "Can't eliminate cycles of length four with this many "
                        "checks per bit\n");
        exit(1);
    }
...
void usage(void) {
    fprintf(stderr,
            "Usage:  make-ldpc pchk-file n-checks n-bits seed method\n");
...
void make_ldpc(int seed,           /* Random number seed */
    ...
    if (no4cycle) {
        ...
        for (t = 0; t < 10; t++) {
            k = 0;
            for (j = 0; j < N; j++) {
                for (e = mod2sparse_first_in_col(H, j); !mod2sparse_at_end(e);
                     e = mod2sparse_next_in_col(e)) {
                    for (f = mod2sparse_first_in_row(H, mod2sparse_row(e));
                         !mod2sparse_at_end(f); f = mod2sparse_next_in_row(f)) {
                        if (f == e)
                            continue;
                        for (g = mod2sparse_first_in_col(H, mod2sparse_col(f));
                            ...
                            if (g == f)
                                continue;
                            for (h = mod2sparse_first_in_row(H,
                                                             mod2sparse_row(g));
                                 !mod2sparse_at_end(h);
        ...
        if (elim4 > 0) {
            fprintf(stderr,
                    "Eliminated %d cycles of length four by moving checks "
                    "within column\n",
                    elim4);
        }
