...
mod2dense *mod2dense_allocate(int n_rows, /* Number of rows in matrix */
    ...
    if (n_rows <= 0 || n_cols <= 0) {
        fprintf(stderr,
                "mod2dense_allocate: Invalid number of rows or columns\n");
        exit(1);
    }
...
void mod2dense_free(mod2dense *m /* Matrix to free */
) {
    free(m->bits);
    free(m->col);
    free(m);
...
void mod2dense_copy(mod2dense *m, /* Matrix to copy */
...
void mod2dense_copyrows(mod2dense *m, /* Matrix to copy */
                        mod2dense *r, /* Place to store copy of matrix */
    ...
    if (mod2dense_cols(m) > mod2dense_cols(r)) {
        fprintf(stderr, "mod2dense_copyrows: Destination matrix has fewer "
                        "columns than source\n");
        exit(1);
    }
...
void mod2dense_copycols(mod2dense *m, /* Matrix to copy */
                        mod2dense *r, /* Place to store copy of matrix */
    ...
    if (mod2dense_rows(m) > mod2dense_rows(r)) {
        fprintf(stderr, "mod2dense_copycols: Destination matrix has fewer rows "
                        "than source\n");
        exit(1);
    }
...
int mod2dense_get(mod2dense *m, /* Matrix to get element from */
                  int row,      /* Row of element (starting with zero) */
                  int col       /* Column of element (starting with zero) */
) {
    if (row < 0 || row >= mod2dense_rows(m) || col < 0 ||
        col >= mod2dense_cols(m)) {
...
void mod2dense_set(mod2dense *m, /* Matrix to modify element of */
                   int row,      /* Row of element (starting with zero) */
                   int col,      /* Column of element (starting with zero) */
...
int mod2dense_flip(mod2dense *m, /* Matrix to flip element in */
                   int row,      /* Row of element (starting with zero) */
                   int col       /* Column of element (starting with zero) */
) {
    mod2word *w;
...
void mod2dense_add(mod2dense *m1, /* Left operand of add */
...
void mod2dense_multiply(mod2dense *m1, /* Left operand of multiply */
    ...
    if (r == m1 || r == m2) {
        fprintf(stderr, "mod2dense_multiply: Result matrix is the same as one "
                        "of the operands\n");
        exit(1);
    }
...
int mod2dense_invert(
    ...
    if (mod2dense_rows(r) != n || mod2dense_cols(r) != n) {
        fprintf(stderr, "mod2dense_invert: Matrix to receive inverse has wrong "
                        "dimensions\n");
        exit(1);
    }
...
int mod2dense_invert_selected(
    ...
    if (r == m) {
        fprintf(stderr, "mod2dense_invert_selected2: Result matrix is the same "
                        "as the operand\n");
        exit(1);
    }
    ...
    if (mod2dense_rows(r) != n || mod2dense_cols(r) != n2) {
        fprintf(stderr, "mod2dense_invert_selected2: Matrix to receive inverse "
    ...
    for (;;) {
        while (i < n - R) {
            ...
            for (j = i; j < n2; j++) {
                if (mod2_getbit(m->col[cols[j]][k0], b0))
                    break;
            }
...
int mod2dense_forcibly_invert(
    ...
    if (r == m) {
        fprintf(stderr, "mod2dense_forcibly_invert: Result matrix is the same "
                        "as the operand\n");
        exit(1);
    }
    ...
    if (mod2dense_rows(r) != n || mod2dense_cols(r) != n) {
        fprintf(stderr, "mod2dense_forcibly_invert: Matrix to receive inverse "
                        "has wrong dimensions\n");
        exit(1);
    }
