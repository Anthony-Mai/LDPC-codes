...
int main(int argc, char **argv) {
    ...
    if (!(pchk_file = argv[1]) || !(gen_file = argv[2]) ||
        !(source_file = argv[3]) || !(encoded_file = argv[4]) || argv[5]) {
        usage();
    }

    if ((strcmp(pchk_file, "-") == 0) + (strcmp(gen_file, "-") == 0) +
            (strcmp(source_file, "-") == 0) >
        1) {
        fprintf(stderr,
    ...
    if (N <= M) {
        fprintf(stderr,
                "Can't encode if number of bits (%d) not greater than number "
                "of checks (%d)\n",
                N, M);
        exit(1);
    }
    ...
    for (n = 0;; n++) {
        ...
        for (i = 0; i < M; i++) {
            if (chks[i] == 1) {
                fprintf(
                    stderr,
                    "Output block %d is not a code word!  (Fails check %d)\n",
                    n, i);
                abort();
            }
        }
